...
CATEGORIES = [
    ...
    ("碎碎念衛教專區", ["衛教", "保養", "建議", "日常", "飲食", "睡眠", "中暑", "養生", "預防", "注意"]),
]
...
CATEGORY_OVERRIDES = {
    ...
    # → 中醫衛教
    "post-2023-08-22": "碎碎念衛教專區",       # 整脊屬醫療行為之安全宣導
    "post-2024-03-23": "碎碎念衛教專區",       # 顱初結構還原：顱骨系統衛教長文
    "post-2025-05-07": "碎碎念衛教專區",       # 「喀」之前先排除風險之安全宣導
    ...
    # ── 網站原生新文章 ──
    "post-2026-07-22": "碎碎念衛教專區",       # 髖關節夾擠與發炎衛教（首篇原生文章）
    # ── 2026-07-21 醫案/衛教全量複審（醫師核可）：關鍵字誤入醫案/衛教者歸位 ──
    "post-2023-05-09": "醫案分享",       # 國中生步態分析案（原誤入衛教）
    "post-2024-08-15": "碎碎念衛教專區",       # 「治療＋訓練」理念衛教（原誤入醫案）
