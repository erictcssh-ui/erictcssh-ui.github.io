...
CATEGORIES = [
    ...
    ("衛教碎碎念", ["衛教", "保養", "建議", "日常", "飲食", "睡眠", "中暑", "養生", "預防", "注意"]),
]
...
TAG_OVERRIDES = {
    "post-2026-08-16": ["耳鳴・耳悶", "顱頸整合", "徒手治療"],  # 耳咽管開放症：口腔內＋高位頸椎＋顳骨三方向
    ...
    # ── 2026-07-23 全站標籤總稽核：部位/症狀標籤限臨床內容，課程筆記不掛部位標 ──
    "post-2023-08-13": [],                                      # Gibbons骨盆課心得，移除下背/骨盆
    "post-2025-03-01": [],                                      # Neumann髖課Day1短文
    "post-2025-03-02": [],                                      # Neumann髖課重點筆記，非臨床
    "post-2025-11-25": [],                                      # PNF骨盆工作坊課程
    "post-2024-05-08": ["徒手治療"],                            # 讀書會骨盆評估分享，移除骨盆部位標
    "post-2025-04-28": ["乾針／超音波針刀"],                    # DN Advanced乾針課，誤掛徒手
    "post-2024-10-24": ["產後・孕期", "徒手治療"],              # 產後媽媽觀念文，肩頸僅帶過
    "post-2026-07-22": ["髖・腹股溝", "骨盆", "徒手治療", "乾針／超音波針刀"],  # 髖夾擠診療紀錄（醫師核定補療法標）
    "post-2026-07-23": ["薦髂關節", "骨盆", "乾針／超音波針刀", "徒手治療"],    # 薦髂沾黏針刀案，補核心部位標
    "post-2026-07-26": ["顳顎關節", "乾針／超音波針刀", "表面解剖"],            # 乾針工作坊助教＋TMJ即席案（醫師核定）
}

# 人工核定分類（2026-07-21 隨筆81篇逐篇審核）：優先於自動關鍵字規則
CATEGORY_OVERRIDES = {
    "post-2026-07-26": "教學與工作坊",   # 乾針工作坊助教記事（2026-07-26 原生新文）
    "post-2026-08-06": "進修筆記",       # 傅士豪 MET 課後臨床應用（自動分類誤判為教學與工作坊）
    "post-2026-08-09": "進修筆記",       # DGSA 乾針治療師認證三年歷程（自動分類誤判為醫案分享）
    # → 醫案分享
    ...
    # → 中醫衛教
    "post-2023-08-22": "衛教碎碎念",       # 整脊屬醫療行為之安全宣導
    "post-2024-03-23": "衛教碎碎念",       # 顱初結構還原：顱骨系統衛教長文
    "post-2025-05-07": "衛教碎碎念",       # 「喀」之前先排除風險之安全宣導
    # ── 2026-07-23 衛教分類複審：敘事主體為個案者歸醫案 ──
    "post-2025-06-12": "醫案分享",       # 突發性暈眩體能訓練師案
    "post-2024-07-27": "醫案分享",       # 兩年下背痛骨盆校正案
    "post-2023-08-17": "醫案分享",       # 膝蓋ACL斷裂轉診案
    "post-2024-12-12": "醫案分享",       # OSA跨科協作案
    "post-2025-07-09": "醫案分享",       # 助理右上腹痛徒手處理案
    # ── 網站原生新文章 ──
    "post-2026-07-22": "衛教碎碎念",       # 髖關節夾擠與發炎衛教（首篇原生文章）
    # ── 2026-07-21 醫案/衛教全量複審（醫師核可）：關鍵字誤入醫案/衛教者歸位 ──
    "post-2023-05-09": "醫案分享",       # 國中生步態分析案（原誤入衛教）
    "post-2024-08-15": "衛教碎碎念",       # 「治療＋訓練」理念衛教（原誤入醫案）
...
EXCERPT_OVERRIDES = {
    "post-2026-07-26": "工作坊尾聲的意外插曲：5 分鐘、2 針，學員多年的顳顎關節卡卡瞬間順暢。看似戲劇性的變化背後沒有魔術——是解剖學、肌動學與生物力學的扎實地基。",
...
EXCLUDE_POSTS = {
    ...
    "post-2024-01-29",   # 新醫師加入公告（時效已過，2026-07-26 醫師指示刪）
...
CONDITION_PAGES = {
    ...
    "hip-impingement.html": ("髖關節夾擠",
                             "蹲不下去、盤腿卡卡、鼠蹊深處痛。夾擠常是結果——腰椎、骨盆、髖這組齒輪先卡住了。"),
    "knee-pain.html": ("膝蓋痛",
...
TAG_INTROS = {
    "骨盆": "調骨盆，不是把骨頭「喬正」到某個絕對正確的位置——而是調「骨盆的回正能力」，把能隨日常活動變化調節的活動能力找回來。骨盆是身體的避震器與力量傳遞的中繼站，活動力喪失的原因可能在關節囊韌帶、肌肉筋膜張力或關節錯移，需要精密的檢查與診斷，而非一體適用的快速手法。以下是診間關於骨盆的觀察與紀錄。",
    ...
    "下背・腰": "腰椎的先天設計是「穩定軀幹」；當胸椎卡住、骨盆失能，本該穩定的腰就被迫代償活動，久了痠痛通通一起來。久坐不動會讓韌帶疲乏（潛變）、保護反射失靈——有一種「緊」其實是「累」，越拉筋反而越痛。修復失能的源頭，而不是只放鬆痛處，是處理下背痛的核心思路。",
    ...
    "顳顎關節": "顳顎關節（TMJ）的痠痛，兇手常常不在關節本身。若張口順暢、路徑正直，「發炎」往往只是結果——真正的拉力可能來自顱骨排列、頸部肌群張力，甚至更遠端的結構。從全身大結構下手，而不是只盯著局部關節，是處理顳顎問題的基本視角。",
    ...
    "耳鳴・耳悶": "對於耳鳴，始終不敢妄言「完全治癒」。但檢查正常、被告知「學著和平共處」的耳鳴耳悶，仍有可以努力的空間——耳道聽骨與耳周肌群連著顳骨，顱骨排列、頸椎筋膜張力都可能參與其中。透過內傷併治的整合處置，讓聲音變小、穩定在可忽略的日常，是目前能盡的最大努力。",
    "產後・孕期": "懷孕與生產，是骨盆帶承受最大變化的時期。產後的腰痛、恥骨痛、尾椎不適，多與薦髂關節、恥骨聯合未回到該有的活動狀態有關；剖腹產的疤痕也可能抑制核心發力。處理方式是徒手與針灸並用，必要時與產後運動、震波等跨專業合作——複雜的問題，本來就需要整合的答案。",
    "乾針／超音波針刀": "乾針是針對激痛點與失能肌筋膜的精準治療——透過準確的評估找到真正的問題肌群，往往幾針就能截斷疼痛的惡性循環。慢性疼痛還會引發中樞敏感化，此時配合中藥的化學性調理雙管齊下更有效率；深層頑固的沾黏，則輔以超音波導引針刀精準處理。持有瑞士 D.G.S.A 官方認證，以下是相關的臨床紀錄。",
    "徒手治療": "徒手治療是投入最深的治療方式，原因很簡單：只要有雙手，就能處理大部分筋骨疼痛的問題。從中醫骨傷科到 MET、SCS、PNF、顱薦與疤痕鬆解，各門各派殊途同歸——重點從來不是把骨頭「喬正」，而是恢復結構該有的功能與活動能力。以下是相關的徒手治療紀錄與學習軌跡。",
    ...
    "顱頸整合": "頭顱與顏面由二十二塊骨頭組成，顱縫在教科書上歸為「不可動關節」，其間卻存在微小而精細的活動。長期的姿態與筋膜張力不均，會默默改變顱骨系統的平衡，衍生慢性頭痛、單側鼻塞、顳顎障礙、大小臉等看似無關的問題。處理顱骨用的是精細輕巧的手法——死推硬擠沒有作用，只會造成傷害。以下是顱頸領域的紀錄與思考。",
    "落枕": "偶爾一次落枕，多半休息幾天就過去；反覆發作的落枕，就值得當作警訊——脖子常只是承受結果的地方，肩胛與背部支撐系統的失衡，才是讓頸部一再「閃到」的原因。處理當下的疼痛之外，找出反覆發作的源頭，才能真正斷根。",
    "中藥調理": "在這套診療架構裡，徒手與針刺處理「物理性」的結構問題，中藥則負責「化學性」的體質調理——慢性疼痛引發的中樞敏感化、產後的氣血修復、睡眠與腸胃的失衡，都是方藥著力的地方。物理性與化學性雙管齊下，常比單打獨鬥更有效率，也是中醫師「內傷併治」的優勢所在。",
    ...
    "表面解剖": "表面解剖與觸診，是針傷徒手醫師的基本功——沒有精準的觸診與定位，再華麗的針法或手法都會失去準星。面對有溫度、有厚度的人體，把每一塊骨頭的相對位置、每一條肌肉的走向與張力一一摸清楚，是開設工作坊、回校園分享時最想傳遞的核心能力。",
...
''' for g, (t, d) in CONDITION_PAGES.items())}      </div>
    ...
    <details class="home-fold">
      <summary>
        ...
        <span class="hf-note">從歷年診間紀錄中選出的代表性案例，帶您認識背後的診療思路。</span>
      </summary>
      <div class="fold-body">
...
{featured_items}
    ...
    <details class="home-fold">
      <summary>
        <span class="hf-head"><span class="hf-title">形氣之間</span><span class="hf-count">系列專題</span></span>
        <span class="hf-note">解剖與經絡的對話——同一塊肌肉，兩種讀法：重疊在哪裡，分歧在哪裡。</span>
      </summary>
      <div class="fold-body">
      <div class="guide-grid">
        <a class="guide-card" href="atlas/gluteus-medius.html">
          <span class="gc-title">臀中肌 Gluteus Medius</span>
          <span class="gc-desc">走路時撐住骨盆的那條肌肉。它失能時痛在下背與薦髂，而不是它自己。對照足少陽膽經的居髎與環跳。</span>
        </a>
        <a class="guide-card" href="atlas/sternocleidomastoid.html">
        ...
        <a class="guide-card" href="atlas/infraspinatus.html">
          <span class="gc-title">棘下肌 Infraspinatus</span>
          <span class="gc-desc">痛在肩膀前面，源頭在肩胛骨後面。對照天宗——古今按的是同一塊肌肉，但痛的方向，兩張地圖畫反了。</span>
        </a>
      </div>
...
""", encoding="utf-8")
    ...
    static_rels = ["", "articles/index.html", "services.html", "faq.html", "courses.html",
                   ...
                   "atlas/masseter.html", "atlas/quadratus-lumborum.html", "atlas/iliopsoas.html", "atlas/infraspinatus.html",
